...
# The names of any people you worked with for this assignment: Madi Atkins 
...
table_spec = "CREATE TABLE IF NOT EXISTS Tweets (tweet_id INTEGER PRIMARY KEY, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets INTEGER)"
...
for x in umsi_tweets:
	...
	total_tweets=(tweet_id,author,time_posted,tweet_text,retweets)
...
two="SELECT * FROM Tweets WHERE retweets>2"
cur.execute(two)
...
first="SELECT tweet_text FROM Tweets WHERE tweet_text LIKE 'RT%'"
cur.execute(first)
